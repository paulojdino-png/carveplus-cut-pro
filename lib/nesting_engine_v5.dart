    ...
    for (final part in expanded) {
      ...
      while (!placed) {
        ...
        for (final sheet in sheets) {
          debugPrint(
            'Searching for ${part.name}: '
            '${part.width} x ${part.height} '
            '(search = ${part.width + settings.partSpacing} x ${part.height + settings.partSpacing})',
          );
          var candidate = _findNode(sheet.freeRects, part.width, part.height);

          bool candidateRotated = false;

          final rotatedCandidate = part.allowRotation
              ? _findNode(sheet.freeRects, part.height, part.width)
              : null;
          if (candidate != null) {
            debugPrint(
              'Found node: '
